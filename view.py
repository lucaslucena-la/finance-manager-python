...
# função para criar uma conta no banco de dados sqlite
...
def transferir_saldo(id_conta_saida, id_conta_entrada, valor): # transferir saldo de uma conta para outra
    with Session(engine) as session: # abre uma sessão
        statement = select(Conta).where(Conta.id==id_conta_saida) # seleciona a conta de saída
        conta_saida = session.exec(statement).first() # executa a query e pega o primeiro resultado

        if conta_saida.valor < valor: # verifica se a conta de saída possui saldo suficiente
            raise ValueError('Saldo insuficiente') 
        statement = select(Conta).where(Conta.id==id_conta_entrada)# seleciona a conta de entrada
...
def movimentar_dinheiro(historico: Historico):
    with Session(engine) as session:
        ...
        if conta.status != Status.ATIVO:
            raise ValueError("Está conta está inativa")
...
def criar_grafico_por_conta():
    with Session(engine) as session: #conexão om o banco 
